...
class Database {
  ...
  private async initializeSchema() {
    try {
      ...
      const existingTables = (tables[0] as any)?.tb || {}
      
      // Only create tables if they don't exist
      if (!existingTables.electricity_price) {
        await this.db.query(`
